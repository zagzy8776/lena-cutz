...
export default function AdminServices() {
  ...
  const fetchData = async () => {
    ...
    } catch (err) { setMessage(err instanceof Error ? err.message : 'Unable to load services.'); }
    finally { setLoading(false); }
  };
  const notify = (text: string) => { setMessage(text); setTimeout(() => setMessage(null), 3000); };

  const addCategory = async () => {
    const name = categoryName.trim(); if (!name) return; setCategorySaving(true);
    try { const category = await api.categories.create(name); setCategories(prev => [...prev, category].sort((a,b) => a.name.localeCompare(b.name))); setCategoryName(''); if (!newCategory) setNewCategory(category.id); notify('Category created.'); }
    catch (err) { notify(err instanceof Error ? err.message : 'Could not create category.'); } finally { setCategorySaving(false); }
  };
  const saveCategory = async (id: string) => {
    const name = categoryEditName.trim(); if (!name) return; setCategorySaving(true);
    try { const updated = await api.categories.update(id, name); setCategories(prev => prev.map(c => c.id === id ? { ...c, ...updated } : c)); setServices(prev => prev.map(s => s.category_id === id ? { ...s, category_name: updated.name } : s)); setEditingCategory(null); notify('Category updated.'); }
    catch (err) { notify(err instanceof Error ? err.message : 'Could not update category.'); } finally { setCategorySaving(false); }
  };
  const deleteCategory = async (category: Category) => {
    if ((category.service_count || 0) > 0) { notify('Move or delete the services in this category first.'); return; }
    if (!confirm(`Delete “${category.name}”?`)) return; setCategorySaving(true);
    try { await api.categories.delete(category.id); setCategories(prev => prev.filter(c => c.id !== category.id)); if (newCategory === category.id) setNewCategory(''); notify('Category deleted.'); }
    catch (err) { notify(err instanceof Error ? err.message : 'Could not delete category.'); } finally { setCategorySaving(false); }
  };
  const updateService = async (id: string, updates: Partial<Service>) => {
    setSaving(id); setSaved(null);
    try { const updated = await api.services.update(id, updates); setServices(prev => prev.map(s => s.id === id ? { ...s, ...updated } : s)); setSaved(id); setTimeout(() => setSaved(null), 2000); }
    catch (err) { notify(err instanceof Error ? err.message : 'Could not save service.'); } finally { setSaving(null); }
  };
  const addService = async () => {
    if (!newName.trim() || !newPrice.trim()) { notify('Service name and price are required.'); return; }
    setSaving('new'); const maxSort = Math.max(...services.map(s => s.sort_order), 0);
    try {
      ...
      setServices(prev => [...prev, data]); setNewName(''); setNewDesc(''); setNewPrice(''); setNewDuration('30'); setNewImage(null); setShowAdd(false); notify('Service added to the website.');
    } catch (err) { notify(err instanceof Error ? err.message : 'Could not add service.'); } finally { setSaving(null); }
  };
  const deleteService = async (id: string) => {
    if (!confirm('Delete this service? Existing bookings will keep their records.')) return; setSaving(id);
    try { await api.services.delete(id); setServices(prev => prev.filter(s => s.id !== id)); notify('Service deleted.'); }
    catch (err) { notify(err instanceof Error ? err.message : 'Could not delete service.'); } finally { setSaving(null); }
  };
